        ...
        with tf.io.TFRecordWriter(tfr_file, options=tf_opts) as writer:
            ...
            for index, row in sub_interval.iterrows():
                ...
                if (len(tss_subints_df.index) == 0):
                    ...
                    tss_tokens = utils.get_TSS_token(interval,tss_subints_df, 
                                                     args.num_bins,args.output_res, args.shift_amt)
                    

                for idx in np.nonzero(tss_tokens)[0]:
                    if idx - 1 >= 0:
                        tss_tokens[idx-1] = 1
                    if idx + 1 < len(tss_tokens):
                        tss_tokens[idx+1] = 1
                ...
                feature = {
                            ...
                            'tss_tokens': _tensor_feature_int(tss_tokens),
